...
def five(input):
	text = "Bert,helloWorld.py,True,1447,Bert,strings.py,False,1318,Jeff,dice.py,False,1445"
	text = text.split(',')
...
def six(input):
	...
	if (f"find '{"cei"}' = {input.find("cei")}"):
		return True
	if (f"find '{"ie"}' = {input.find("ie")}"):
		return True
	if (f"find '{"cie"}' = {input.find("cie")}"):
		return False
	if (f"find '{"ei"}' = {input.find("ei")}"):
		return False
...
def nine(inputString, char):
	for i in range(0, len(inputString)):
		...
		return ("character {} is present at {}".format(char, str(res)))
